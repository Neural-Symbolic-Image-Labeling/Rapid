...
def foil_gain(pre_p, pre_n, now_p, now_n):
    ...
    gain = now_p * (math.log2(now_p / (now_n + now_p)) - math.log2(
        pre_p / (pre_p + pre_n)))  # now_p:new positive, pre_n: previous negative
    return gain
...
def get_new_total_list(result_list, total_list):
    ...
    for i in range(len(del_number)):
        del new_total[
            del_number[len(del_number) - 1 - i]]  # the position is in positive sequence, first delete the back one
...
def get_possible_clause1(counting, total_list, result_list):
    if counting == 0:
        ...
        for image in new_total:
            for i, clause in enumerate(image):
                if i != 0 and (
                        clause not in clause_total):  # The first position of each image is classification, so clauses start from the second position
        ...
        for image in new_total:
            for i, clause in enumerate(image):
                if i != 0 and (clause not in result_list[len(result_list) - 1]) and (
                        clause not in clause_total):  # The first position of each image is classification, so clauses start from the second position
                    clause_total.append(clause)
    return clause_total
...
def get_total_list1(input_list):
    ...
    for image_num in range(len(input_list)):
        ...
        for index, objects in enumerate(position_list1):
            has = total_object[objects] + "(image" + str(input_list[image_num]['imageId']) + "," + str(objects) + ")"
            area = "area" + "(" + str(objects) + "," + str(
                input_list[image_num]['panoptic_segmentation'][str(index)]['area']) + ")"
            image_list.append(has)
        ...
        for objects_num in range(len(input_list[image_num]['object_detect']['overlap'])):
            object1_name = input_list[image_num]['object_detect']['object'][
                str(input_list[image_num]['object_detect']['overlap'][str(objects_num)]["idA"])]['name']
            object2_name = input_list[image_num]['object_detect']['object'][
                str(input_list[image_num]['object_detect']['overlap'][str(objects_num)]["idB"])]['name']
...
def foil(target_list, target, total_list, variable1, variable2, deleted,
         locked):  # target should be a string, such as "guitarist"
    ...
    positive_list, negative_list = pos_neg_list(target,
                                                new_total_list)  # get the initial_positive_list,to help find out the result that can satisfy all the positives
    c = re.split(r'[(|,|)]', target)
    ...
    while (len(positive_list) != 0):
        ...
        while (len(negative_list) != 0):
            ...
            # TODO: to use multiprocessing to parallelize the following for loop
            for new_clause in possible_clause:
                # calculate the new possible clause foil_gain
                now_p = now_n = 0
                for image_number, image in enumerate(
                        new_total_list):  # now_p (now_n) is the num of positive (negative) images containing such a object depicted with ``new_clause''
                    for clause in image:
                        if clause == new_clause:
                            # TODO: to use simpler function to search for the image
            ...
            while correct_clause == False:
                for clause_number, clause_gain in enumerate(
                        foil_gain_list):  # TODO consider to search for predicates in hierarchy, choose thoses with high tf.idf first
                    ...
                    if clause_gain == max(foil_gain_list):
                        ...
                        if a[0] == "overlap":
                            if (a[1] in parameter_list) and (a[2] in parameter_list) and still_has_num(result) >= 2:
                                new_result = copy.deepcopy(
                                    result)  # The following is to first add it to result, if it is a special case
                                new_result.append(possible_clause[
                                                      clause_number])  # such that only one positive has the clause and no negative has it, it may has the best gain.
                            ...
                            if a[1] in parameter_list and threshold(target, possible_clause[clause_number], total_list1,
                                                                    variable1, variable2) != False and still_has_num(
                                    result) >= 3:
            ...
            positive_list, negative_list = pos_neg_list(target,
                                                        new_total_list)  # can use for next iteration when the answer is not perfect
...
def neg_FOIL(input_list, deleted, locked):
    ...
    dict_math = {}
    dict_nl = {}
    total_list1 = get_total_list1(
        input_list)  # TODO duplicated computing here. we can split ``input_list'' into previous list and new list to save time
