    ...
    def split_images(self, data_file, config_file, image_dir, target_path):
        ...
        for img in data:
            if img['imageId'] in X_train_id:
                shutil.copy(f'{image_dir}/{img["type"]}/{img["imageId"]}.jpg', f'{target_path}/train/{img["type"]}/{img["imageId"]}.jpg')
            elif img['imageId'] in X_test_id:
                shutil.copy(f'{image_dir}/{img["type"]}/{img["imageId"]}.jpg', f'{target_path}/test/{img["type"]}/{img["imageId"]}.jpg')
            elif img['imageId'] in X_val_id:
                shutil.copy(f'{image_dir}/{img["type"]}/{img["imageId"]}.jpg', f'{target_path}/val/{img["type"]}/{img["imageId"]}.jpg')


if __name__ == '__main__':
    data_parser = ClassificationDataManager()
    result = data_parser.split_data(data_file='FOIL/data_file/bird/bird_168169170.json', config_file='FOIL/data_file/bird/config_bird_168169170.json')
    print(len(result[0]), len(result[1]), len(result[2]), len(result[3]), len(result[4]), len(result[5]))
