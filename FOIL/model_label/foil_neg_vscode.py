        ...
        if image[0]==target:
            for predicate_pos,predicate in enumerate(image):
                if predicate_pos!=0:
                    ...
                    if predicate not in result_list and result<0.01:
    ...
    for image in total_list:
        ...
        for predicate in new:
            ...
            if len(a)==5:
                ...
                if predicate not in new_image:
                    new_image.append(predicate)
            elif a[0]=="color" or a[0]=="area" or a[0]=='num':
...
def get_object_list(total_list):
    ...
    for image in total_list:
        for clauses in image:
            a=re.split(r'[(|,|)]',clauses)
            if a[0]!='overlap' and a[0]!='num' and a[0]!='color' and a[0]!='area' and len(a)==4 and (a[0] not in object_list):
    ...
    for rule in lock[c[0]]:
        if rule[2]==1:
            for i,image in enumerate(new_total_list):
                satisfy_list=["False" for i in range(len(rule[1]))]
                object_in_rule=[]
                object_character=[]
                for position,clauses in enumerate(rule[1]):
                    a=re.split(r'[¬|(|,|)]',clauses)
                    if a[0]!='overlap' and a[0]!='num' and a[0]!='area' and a[0]!='color' and len(a)==4:
                        ...
                        for predicate in image:
                            b=re.split(r'[(|,|)]',predicate)
                            if b[0]!='overlap' and b[0]!='num' and b[0]!='area' and b[0]!='color':
                        ...
                        for predicate in image:
                            ...
                            if b[0]==a[0] and b[2]==a[2]:
                                object1_in_image=object_in_image[object_number.index(b[1])]
                                if object1_in_image==object1_in_rule:
                                    satisfy_list[position]="True"
                                    break
        ...
        for rule in rules:
            if rule!=0 and rule!=1:
                ...
                for pos,predicate in enumerate(rule):
                    ...
                    elif a[0]=='num' or a[0]=='area' or a[0]=='color':
                        a[1]=str(object_list.index(a[1]))
                        rule[pos]=a[0]+'('+a[1]+','+a[2]+')'+a[3]
...
def multiprocess(new_clause,new_total_list,positive_list,negative_list):
    now_p=now_n=0
            ...
            if clause==new_clause:
                ...
                for negative_image_number in negative_list:
                    if image_number==negative_image_number:
                        now_n+=1
    result=foil_gain(len(positive_list),len(negative_list),now_p,now_n)
    return result

def getting(possible_clause,new_total_list,positive_list,negative_list):
    num_worker = 16  # 线程池数量
    result_list = []  # 存储多线程的返回值
    pool = Pool(num_worker)
    for i, indis in enumerate(possible_clause): 
        result_list.append(pool.apply_async(func=multiprocess, args=(indis, new_total_list, positive_list,negative_list)))
    pool.close()
    ...
    for i in result_list:
        foil_gain_list.append(i.get())
    # res即函数的返回值
    return foil_gain_list
...
def foil(target_list,target,total_list,variable1,variable2,deleted,locked):         #target should be a string, such as "guitarist"
    ...
    while (len(positive_list)!=0):
        ...
        while (len(negative_list)!=0):
            ...
            #start=time.time()
            #foil_gain_list=getting(possible_clause,new_total_list,positive_list,negative_list)
                    ...
                    for predicate in rule[0]:
                        if predicate not in result_list[i]:
                            ...
                            if a[0]!='num' and a[0]!='area':
                                result_in=False
                                break
                            else:                    # num and area in our design is num(1,1.7), so just check num and 1
                                result_ins=False
